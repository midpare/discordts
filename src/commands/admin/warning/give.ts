...
export default new Command({
  ...
  execute: async ({ msg, args }) => {
    ...
    if (!target)
      return msg.reply(message.admin.warning.give.missingMentionUser);
    ...
    if (count > 10)
      return msg.reply(message.admin.warning.give.overNumber);
    ...
    if (!user) {
      ...
      await newUser.save();
      channel.send(message.admin.warning.give.success(target.user, count, count, reason));
      return;
    }

    (await warning.updateOne({ id }, { $inc: { warning: count } }, { upsert: true })).matchedCount;
    channel.send(message.admin.warning.give.success(target.user, count, user.warning + count, reason));
  },
});